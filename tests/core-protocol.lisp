...
(deftest display-protocol ()
  ...
  (let ((display (xlib:open-default-display)))
    ...
    ;; display-plist
    (is (setf (getf (xlib:display-plist display) :foo) :bar))
    (is (eql :bar (getf (xlib:display-plist display) :foo)))
    (is (remf (xlib:display-plist display) :foo))
...
(deftest screen-protocol ()
  ...
  (with-default-display display
    (let ((screen (xlib:display-default-screen display)))
      ...
      (is (setf (getf (xlib:screen-plist screen) 'foo) "hell is empty"))
  ...
  (with-default-display display
    (let* ((screen (xlib:display-default-screen display))
      ...
      (dolist (drawable (list root-window child-window pixmap))
        ...
        (is (setf (getf (xlib:drawable-plist drawable) 'foo) "hell is empty"))
      ...
      (dolist (window (list root-window child-window))
        ...
        (is (setf (xlib:window-background window) pixmap))
        (is (setf (xlib:window-background window) #xffffff))
        (is (setf (xlib:window-background window) :none))
        (is (setf (xlib:window-background window) :parent-relative))
        ...
        (is (setf (getf (xlib:window-plist window) 'foo) "hell is empty"))
        (is (string= "hell is empty" (getf (xlib:window-plist window) 'foo)))
        (is (member (xlib:window-save-under window) '(:on :off)))
        (istype 'xlib:card29 (xlib:window-visual window)))
      (isnt (xlib:window-equal child-window root-window))
      (is= (xlib:window-backing-planes child-window) #xffffffff)
      (isequal (xlib:window-backing-store child-window) :not-useful)
      ...
      (is (xlib:circulate-window-down child-window))
      (is (xlib:circulate-window-up child-window))
      ...
      (is (setf (xlib:drawable-border-width child-window) 2))
      ...
      (is (setf (xlib:drawable-width child-window) 120))
      (is (setf (xlib:drawable-height child-window) 120))
      (is (setf (xlib:drawable-x child-window) 60))
      (is (setf (xlib:drawable-y child-window) 70))
      (is (setf (xlib:window-backing-pixel child-window) #xAAAAAA))
      (is (setf (xlib:window-backing-planes child-window) #xBBBBBB))
      (is (setf (xlib:window-backing-store child-window) :when-mapped))
      (is (setf (xlib:window-bit-gravity child-window) :south-west))
      (is (setf (xlib:window-border child-window) #xCCCCCC))
      ;; TODO: setfing colormaps, cursors
      (is (setf (xlib:window-do-not-propagate-mask child-window) 4))
      (is (setf (xlib:window-event-mask child-window) 5))
      ...
      (xlib:with-state (child-window2)
        (is (setf (xlib:drawable-width child-window2) 120))
        (is (setf (xlib:drawable-height child-window2) 120))
        (is (setf (xlib:drawable-x child-window2) 60))
        (is (setf (xlib:drawable-y child-window2) 70))
        (is (setf (xlib:window-backing-pixel child-window2) #xAAAAAA))
        (is (setf (xlib:window-backing-planes child-window2) #xBBBBBB))
        (is (setf (xlib:window-backing-store child-window2) :when-mapped))
        (is (setf (xlib:window-bit-gravity child-window2) :south-west))
        (is (setf (xlib:window-border child-window2) #xCCCCCC))
        (is (setf (xlib:window-do-not-propagate-mask child-window2) 4))
        (is (setf (xlib:window-event-mask child-window2) 5))
      ...
      (is (xlib:map-window child-window))
      (is (xlib:map-subwindows root-window))
      (is (xlib:unmap-window child-window))
      (is (xlib:unmap-subwindows root-window))
      (is (xlib:free-pixmap pixmap))
      (is (xlib:destroy-window child-window))
      ...
      (is (xlib:destroy-subwindows root-window)))))
...
(deftest graphics-contexts ()
  (with-default-display display
    (let* ((screen (xlib:display-default-screen display))
      ...
      (is (setf gc (xlib:create-gcontext :drawable root-window)))
