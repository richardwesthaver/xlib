...
;; CLX can maintain a mapping from X server ID's to local data types. If one
;; takes the view that CLX objects will be instance variables of objects at
;; the next higher level, then PROCESS-EVENT will typically map from
;; resource-id to higher-level object. In that case, the lower-level CLX
;; mapping will almost never be used (except in rare cases like query-tree),
;; and only serve to consume space (which is difficult to GC), in which case
;; always-consing versions of the make-<mumble>s will be better. Even when
;; maps are maintained, it isn't clear they are useful for much beyond xatoms
;; and windows (since almost nothing else ever comes back in events).
